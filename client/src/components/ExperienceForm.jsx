...
const ExperienceForm = ({data, onChange}) => {
  ...
  const [generatingIndex, setGeneratingIndex] = React.useState(-1);
    ...
    const generateDescription=async(index)=>{
      ...
      const prompt=`enhance this job descriptioin ${experience.description} for the position of ${experience.position} at ${experience.company}`
      ...
      try {
        ...
        // Server returns 'enhancedSummary' per controller; use that to update description
                ...
                <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                    ...
                    <div className='grid  md:grid-cols-2 gap-3'>
                        ...
                        <input value={experience.position || ''} onChange={(e)=>updateExperience(index, 'position', e.target.value)} type="text" placeholder='Job Title' className='px-3 py-2 text-sm rounded-lg'/>
                          ...
                          <input value={experience.end_date || ''} onChange={(e)=>updateExperience(index, 'end_date', e.target.value)} type="month"  className='px-3 py-2 text-sm rounded-lg disabled:bg-gray-100' disabled={experience.is_current}/>
                    </div>
                    ...
                    <div className='space-y-2'>
                      ...
                      <textarea value={experience.description || ''} onChange={(e)=>updateExperience(index, 'description', e.target.value)} rows={4} className='w-full text-sm px-3 py-2 rounded-lg resize-none' placeholder='Describe your key responsibilities and achivements...'/>
                    </div>
                </div>
            ))}
        </div>
